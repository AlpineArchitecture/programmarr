...
| POST | `/api/pipeline/compose` | **Planner v2 deterministic resolver.** Body `ComposeRequest{specs:[CandidateSpec], start}`. Each `CandidateSpec{kind, …}` (`kind` ∈ genre / genre_decade / blend / studio / director / actor / tv_genre / marathon / tv_movie_mix / **network** / **programming_block**) is resolved against `plex_library.csv` into a title list; empties skipped + reported. `network` matches TV show `Studio` values; `programming_block` intersects spec's `titles` field with library TV show titles. Writes **`channels.draft.json`** with sequential tight-packed numbering. Returns `{count, channels:[{number,name,items}], skipped}`. |
